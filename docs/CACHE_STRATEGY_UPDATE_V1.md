...
### Обзор
План реализации периодической синхронизации включает backend API v0.3 и обновление Android приложения. Frontend (web) использует существующую синхронизацию через WebSocket и не требует изменений.

### 1. Backend API v0.3

#### Новые эндпоинты:
- `POST /api/v0.3/sync/hash-check` - принимает список пар (entity_type, id, hash), возвращает различия
- `GET /api/v0.3/sync/full-state/{entity_type}` - возвращает полное состояние задач/пользователей/групп
- `POST /api/v0.3/sync/resolve-conflicts` - обрабатывает конфликты, возвращает разрешенное состояние

#### Реализация:
- Создать `HashService` для генерации SHA-256 хешей
- Формат хеша: конкатенация полей с разделителем "|", null → ""
- Сериализация должна быть идентичной на клиенте и сервере
- Логика разрешения конфликтов на основе `updated_at` по правилам из раздела "Разрешение конфликтов"

#### Совместимость:
- API v0.2 остается для существующих клиентов
- Новые клиенты используют v0.3

### 2. Android: Полная реализация синхронизации

#### 2.1. Миграция хранилища
- Переход от SharedPreferences к Room Database
- Создание Entity классов:
  - `TaskEntity` - задачи
  - `UserEntity` - пользователи
  - `GroupEntity` - группы
- Репозитории: `TaskRepository`, `UserRepository`, `GroupRepository`
- Миграция данных при обновлении приложения

#### 2.2. Синхронизация
- `SyncWorker` (WorkManager) - периодическая сверка каждый час
- `ConnectionMonitor` - отслеживание состояния сети
- Логика запуска:
  - Каждый час при наличии сети
  - При восстановлении связи: если время синхронизации уже прошло - немедленно
- `HashGenerator` - генерация хешей для локальных данных
- `SyncQueue` - очередь операций для оффлайн

#### 2.3. Разрешение конфликтов
- Получение различий от `/api/v0.3/sync/hash-check`
- Применение алгоритмов разрешения (последнее изменение побеждает)
- Обновление локального состояния
- Уведомление пользователя при конфликтах

#### 2.4. Архитектура компонентов
```
SyncManager
├── SyncScheduler (планировщик)
├── SyncWorker (фоновый воркер)
├── HashGenerator (генератор хешей)
├── ConflictResolver (разрешение конфликтов)
└── SyncQueue (очередь операций)
```

### 3. Порядок внедрения

1. **Фаза 1: Backend v0.3 API**
   - Разработка HashService
   - Добавление эндпоинтов
   - Тестирование генерации хешей

2. **Фаза 2: Android Room миграция**
   - Создание Entity классов
   - Миграция данных из SharedPreferences
   - Тестирование репозиториев

3. **Фаза 3: Android синхронизация**
   - Реализация SyncWorker
   - HashGenerator
   - Логика ConnectionMonitor
   - Тестирование оффлайн/онлайн сценариев

4. **Фаза 4: Разрешение конфликтов**
   - ConflictResolver
   - Интеграция с API
   - Тесты конфликтов

5. **Фаза 5: Интеграционное тестирование**
   - End-to-end тесты
   - Производительность
   - Совместимость
...
6. **Фаза 6: Деплой**
   - Backend v0.3
   - Android update с миграцией

### 4. Риски и mitigation

- **Миграция данных**: Потеря данных при Room переходе
  - Mitigation: backup/restore механизм
- **Производительность**: Хеширование больших объемов
  - Mitigation: оптимизация, кеширование
- **Конфликты**: Непредвиденные сценарии
  - Mitigation: логирование, ручное разрешение
- **Совместимость**: Старые клиенты
  - Mitigation: поддержка v0.2 API

### 5. Метрики успеха

- Синхронизация раз в час работает без ошибок
- Конфликты разрешаются корректно
- Миграция данных проходит без потерь
- Приложение работает оффлайн > 1 час без проблем
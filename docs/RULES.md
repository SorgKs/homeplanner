...
- Backend запускается из корня репозитория, используя существующее venv:
  ...
  - Любые CLI‑флаги (`--host`, `--port`, `--reload` и т. п.) запрещены — сервер читает все параметры только из `config/settings.toml`.
- Frontend отдается как статический контент самим backend (см. `backend/main.py`). Отдельный дев‑сервер фронтенда не обязателен.
- Конфигурация задаётся только через `config/settings.toml`. Переменные окружения и CLI‑флаги для параметров запуска запрещены.
...
- Конфигурация через файлы (`config/settings.toml`), см. `backend/config.py`.

...
- Backend запускается из корня репозитория, используя существующее venv:
  - Предпочтительно: `uv run python -m backend.main`.
  - Допустимо: `uv run uvicorn backend.main:app --reload`.
  - Требование: при запуске не задаём IP/порт через CLI‑флаги (`--host/--port`). Хост и порт берутся из конфигурации (`backend/config.py`) и/или `.env`.
- Frontend отдается как статический контент самим backend (см. `backend/main.py`). Отдельный дев‑сервер фронтенда не обязателен.
- Переменные окружения берутся из `.env` в корне (см. `backend/config.py`). При отсутствии `.env` backend инициализирует его минимальными значениями.
...
- Конфигурация через переменные окружения (`.env`), см. `backend/config.py`.

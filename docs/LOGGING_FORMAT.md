...
**ВАЖНО:** В Android приложении используется **единственная система логирования** - `BinaryLogger`. Стандартный `android.util.Log` **не используется** и полностью заменен на `BinaryLogger` для обеспечения единообразного логирования и автоматической отправки логов на сервер.
...
3. **Данные контекста** (зависят от messageCode):
   ...
   - **ВАЖНО: Все поля контекста обязательны** - при записи должны присутствовать все поля, определенные в схеме для данного кода сообщения
   - Данные записываются подряд в фиксированном порядке, строго соответствующем порядку полей в `context_schema` словаря
   - Типы данных:
...
**ВАЖНО: Обязательность полей контекста:**

- **Все поля, определенные в `context_schema` для кода сообщения, являются обязательными.**
- При логировании на клиенте (Android) все поля схемы должны присутствовать в `Map<String, Any>`, передаваемом в `BinaryLogger.log()`.
- **Порядок ключей в `Map` должен соответствовать порядку полей в схеме** - это обеспечивает читаемость кода и единообразие.
- Порядок записи полей в бинарный поток определяется схемой (`context_schema` на сервере, `ContextSchema.getSchemaOrder()` на клиенте), но для единообразия порядок ключей в `Map` также должен соответствовать схеме.
- Если поле отсутствует в `Map`, кодировщик выбросит исключение `IllegalArgumentException`.
- Это гарантирует, что декодер на сервере всегда читает данные в ожидаемом порядке и с правильными типами.
...
На стороне клиента в коде **нет текстового шаблона**; он указывается **только в комментарии** рядом с вызовом функции логирования:

```kotlin
// Ожидалось %wait%, фактически %fact%
BinaryLogger.getInstance()?.log(5u, listOf(wait, fact))
```

**ВАЖНО:** 
- Функция `BinaryLogger.log()` принимает только числовой код (`UShort`) и список значений контекста (`List<Any>`)
- Текстовые сообщения **не хранятся в коде** - только в комментариях
- Порядок значений в `listOf()` должен соответствовать порядку полей в `context_schema` на сервере
...
**Важно:**
- Параметр `context` принимает `List<Any>` (список значений без ключей)
- **Все поля, определенные в схеме контекста для данного кода сообщения, обязательны** и должны присутствовать в списке
- **Порядок значений в списке должен строго соответствовать порядку полей в `context_schema`** на сервере
- Данные из `context` автоматически конвертируются в бинарный формат при записи в чанк
- Контекст декодируется на сервере и хранится в поле `text` таблицы `debug_logs` (в виде готового текстового сообщения)
- Код сообщения (`code`) - это числовой идентификатор (`UShort`), а динамические данные передаются в `context` как список значений

#### Правила использования контекста

**Запрещено создавать новые параметры специально для целей логирования:**
- Контекст предназначен **исключительно для передачи уже существующих в коде переменных и значений**
- Нельзя создавать искусственные строки или переменные (например, `"Custom message: " + error.message`) только для логирования
- Все значения в `context` должны быть **реальными переменными из текущего контекста выполнения**
- Если необходимо передать дополнительную информацию, следует использовать существующие переменные или добавить их в код заранее

**Правильный пример:**
```kotlin
val errorCode = response.statusCode
val retryCount = currentAttempt
// Используем существующие переменные errorCode и retryCount
BinaryLogger.getInstance()?.log(3u, listOf(errorCode, retryCount))
```

**Неправильный пример:**
```kotlin
// Создание искусственной строки специально для лога - ЗАПРЕЩЕНО!
val logMessage = "Error ${response.statusCode} on attempt ${currentAttempt}"
BinaryLogger.getInstance()?.log(91u, listOf(logMessage, "custom details"))
```
...
1. **BinaryLogger** - единственная система логирования в Android приложении:
   - Метод `log()` принимает код сообщения и контекст (Map<String, Any>)
   - Логи буферизуются и автоматически отправляются на сервер
   - Работает только в дебаг-версиях (`BuildConfig.DEBUG`)
   - **Полностью заменяет стандартный `android.util.Log`** - все логирование в приложении выполняется через `BinaryLogger`
   - Не использует `android.util.Log` для избежания зацикливания и обеспечения единообразия
...
Данные переменных передаются через параметр `context: List<Any>` (список значений без ключей):

```kotlin
// Полный текст сообщения с плейсхолдерами в комментарии
// Ожидалось %wait%, фактически %fact%
BinaryLogger.getInstance()?.log(
    LogMessageCode.ERROR_EXCEPTION,
    listOf(wait, fact)  // Порядок значений соответствует порядку полей в context_schema
)
```

**ВАЖНО:**
- Порядок значений в `listOf()` должен строго соответствовать порядку полей в `context_schema` на сервере
- Текстовое описание сообщения указывается только в комментарии рядом с вызовом
- Контекст автоматически конвертируется в бинарный формат и записывается в чанк
...
- Обработка на сервере:
  ...
  2. Последовательное чтение записей до конца потока:
     - для каждой записи читается:
       ...
       - **ВАЖНО:** Все поля контекста обязательны и читаются строго в порядке `context_schema` из словаря.
  3. По `dictionary_revision` и числовому `messageCode`:
     ...
     - читают значения контекста в порядке, определённом `context_schema`;
...
## Синхронизация схем контекста между клиентом и сервером

### Android: ContextSchema.kt

На клиенте (Android) порядок полей контекста определяется в файле `android/app/src/main/java/com/homeplanner/debug/ContextSchema.kt`:

```kotlin
object ContextSchema {
    fun getSchemaOrder(messageCode: String): List<String>? {
        return when (messageCode) {
            LogMessageCode.SYNC_CACHE_UPDATED -> listOf("tasks_count", "source", "queue_items")
            LogMessageCode.SYNC_START -> listOf("cache_size")
            // ...
        }
    }
}
...
### Backend: debug_log_dictionary.py

На сервере (Python) порядок полей определяется в `context_schema` словаря:

```python
LOG_MESSAGE_DICTIONARY = {
    41: {  # SYNC_CACHE_UPDATED
        "template": "...",
        "level": "INFO",
        "context_schema": [
...
1. **Порядок полей в `ContextSchema.kt` должен точно совпадать с порядком в `context_schema` словаря на сервере.**
2. **Все поля, определенные в схеме, обязательны** - при логировании все поля должны присутствовать в `List<Any>`.
3. **Порядок значений в `List` должен строго соответствовать схеме** - значения в `listOf()` должны быть перечислены в том же порядке, что и поля в схеме.
4. **Типы полей должны совпадать** - тип в `context_schema` должен соответствовать типу значения в `List`.
5. **При изменении схемы на сервере** необходимо обновить `ContextSchema.kt` на клиенте и исправить порядок значений во всех местах логирования.
6. **При добавлении нового кода сообщения** необходимо добавить схему в оба места (клиент и сервер).

### Механизм записи

`BinaryLogEncoder` использует `ContextSchema.getSchemaOrder()` для валидации количества значений:

```kotlin
val schemaOrder = ContextSchema.getSchemaOrder(entry.messageCode)
if (schemaOrder != null) {
    // Проверяем, что количество значений соответствует схеме
    if (entry.context.size != schemaOrder.size) {
        throw IllegalArgumentException("Context size mismatch for message code ${entry.messageCode}")
    }
    // Записываем значения в порядке списка (порядок должен соответствовать схеме)
...
Это гарантирует, что порядок записи на клиенте всегда совпадает с порядком чтения на сервере.

**ВАЖНО:** **Порядок значений в `List` должен строго соответствовать схеме** для:
- **Правильной расшифровки на сервере** - значения читаются в том же порядке, что и записываются
- **Читаемости кода** - явно видно, какие значения и в каком порядке используются
- **Единообразия** - все места логирования следуют одному паттерну

**Правильный пример:**
```kotlin
// Кэш обновлен после синхронизации: %tasks_count% задач из %source%, очередь: %queue_items%
// Порядок значений соответствует схеме: tasks_count, source, queue_items
BinaryLogger.getInstance()?.log(
    LogMessageCode.SYNC_CACHE_UPDATED,
    listOf(
        serverTasks.size,        // 1-е поле схемы (tasks_count: int)
        "syncCacheWithServer",   // 2-е поле схемы (source: string)
        queueSize                // 3-е поле схемы (queue_items: int)
    )
)
```
...
**Неправильный пример:**
```kotlin
// Порядок значений не соответствует схеме - приведет к неправильной расшифровке!
BinaryLogger.getInstance()?.log(
    LogMessageCode.SYNC_CACHE_UPDATED,
    listOf(
        queueSize,                // 3-е поле схемы, но указано первым - ОШИБКА!
        serverTasks.size,        // 1-е поле схемы, но указано вторым - ОШИБКА!
        "syncCacheWithServer"    // 2-е поле схемы, но указано третьим - ОШИБКА!
    )
)
// Сервер прочитает значения в неправильном порядке и расшифрует сообщение неверно!
```

...
- Выполняется периодически (каждые 30 секунд) при наличии интернета (только синхронизация, без обновления UI)
...
- **Важно:** Периодическая синхронизация не обновляет UI напрямую — UI обновляется только по триггерам
...
2. **Синхронизация очереди (каждые 30 секунд):**

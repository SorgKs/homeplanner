...
**Примечание**: Команда выполняется из корня проекта. Если вы работаете вручную и находитесь в другой директории, используйте `cd` для перехода в корень проекта перед выполнением команды.
...
Команды выполняются **ОТДЕЛЬНО** (активация и запуск — это разные команды):
...
- Общие хелперы (работа с API, временем, настройкой клиентских фикстур) выносите в `tests/utils.py` или профильные модули и подключайте через импорт.

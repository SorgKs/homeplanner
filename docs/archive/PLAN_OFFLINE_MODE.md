...
> **Статус:** ✅ **Почти завершён** (5 из 6 этапов завершены)  
> **Последнее обновление:** 2025-01-27  
> **Осталось:** Этап 6 (тестирование) - частично выполнен
...
### Что уже работает:
1. **Offline-first архитектура**: `LocalApi` загружает данные из кэша, `SyncService` синхронизирует в фоне
...
5. **Локальная фильтрация**: `TodayTaskFilter` реализует фильтрацию задач для вкладки "Сегодня"
6. **Пересчет дат**: `TaskDateCalculator` и `OfflineRepository.updateRecurringTasksForNewDay()` реализованы
7. **Синхронизация перед пересчетом**: `SyncService.syncStateBeforeRecalculation()` реализован
...
**Критерии готовности:**
- ✅ Бэкенд использует новые правила фильтрации для `one_time` (требует проверки)
...
**Выполнено:**
- ✅ `TodayTaskFilter.kt` создан и реализован
- ✅ Метод `filterTodayTasks()` работает корректно
- ✅ Интегрирован в `MainActivity` для вкладки "Сегодня"
...
**Выполнено:**
- ✅ `TaskDateCalculator.kt` создан и реализован
- ✅ Все методы (`getDayStart`, `isNewDay`, `calculateNextReminderTime`) работают
- ✅ `OfflineRepository.updateRecurringTasksForNewDay()` реализован
- ✅ Методы для работы с `last_update` timestamp реализованы
...
**Файлы для изменения:**
- `android/app/src/main/java/com/homeplanner/MainActivity.kt` ✅
- ~~`android/app/src/main/java/com/homeplanner/api/TasksApiOffline.kt`~~ (удалён, миграция на LocalApi завершена)
...
2. ~~В `TasksApiOffline.getTasks()`:~~ (устарело - `TasksApiOffline` удалён, используется `LocalApi`)
   - Пересчет выполняется только в `MainActivity.loadTasks()`
...
4. ✅ Создать метод `SyncService.syncStateBeforeRecalculation()`:
   - ✅ Реализован и проверяет наличие операций в очереди
   - ✅ Выполняет синхронизацию очереди через `syncQueue()`
   - ✅ Загружает актуальное состояние задач с сервера через `tasksApi.getTasks(activeOnly=false)`
   - ✅ Сохраняет задачи в кэш через `offlineRepository.saveTasksToCache()`
   - ✅ Возвращает `true` если синхронизация выполнена, `false` если офлайн
...
**Выполнено:**
- ✅ `syncStateBeforeRecalculation()` реализован в `SyncService`
- ✅ Интегрирован в `MainActivity.loadTasks()` (строка 722, 725)
- ✅ Перепланирование уведомлений реализовано
...
**Файлы для изменения:**
- `android/app/src/main/java/com/homeplanner/api/LocalApi.kt` ✅ (вместо TasksApiOffline)
- `android/app/src/main/java/com/homeplanner/MainActivity.kt` ✅

**Задачи:**
1. ✅ В `LocalApi.completeTask()`:
...
**Выполнено:**
- ✅ `ReminderScheduler` реализован
- ✅ Метод `scheduleForTaskIfUpcoming()` проверяет устаревшие даты и пересчитывает
- ✅ Интегрирован в `MainActivity`

---

### Этап 6: Тестирование и отладка ⚠️
...
- ✅ `android/app/src/androidTest/java/com/homeplanner/repository/OfflineRepositoryTest.kt` (содержит тесты для `updateRecurringTasksForNewDay`)
- ✅ `android/app/src/androidTest/java/com/homeplanner/sync/SyncServiceTest.kt` (содержит тесты для `syncStateBeforeRecalculation`)
- ⚠️ `android/app/src/androidTest/java/com/homeplanner/integration/OfflineModeIntegrationTest.kt` (требует проверки)
- ⚠️ `tests/test_task_service.py` - добавить тесты для новых правил фильтрации `one_time` (требует проверки)
...
**Критерии готовности:**
- ✅ Все unit-тесты проходят (для реализованных компонентов)
- ⚠️ Все интеграционные тесты проходят (требует проверки наличия всех тестов)
- ⚠️ Все тесты сложных сценариев синхронизации проходят (требует проверки)
- ⚠️ Все функциональные тесты проходят (требует проверки)
- ⚠️ Тесты на бэкенде проходят (требует проверки обновления правил фильтрации)
- ✅ Производительность приемлемая (пересчет не блокирует UI)
- ✅ Нет регрессий в онлайн-режиме
- ⚠️ Покрытие тестами новых функций > 80% (требует проверки)

**Статус тестов:**
- ✅ `TodayTaskFilterTest.kt` - создан
- ✅ `TaskDateCalculatorTest.kt` - создан
- ✅ `OfflineRepositoryTest.kt` - содержит тесты для `updateRecurringTasksForNewDay`
- ✅ `SyncServiceTest.kt` - содержит тесты для `syncStateBeforeRecalculation`
- ⚠️ `OfflineModeIntegrationTest.kt` - требуется проверка наличия
- ⚠️ Тесты на бэкенде (`test_task_service.py`) - требуется проверка обновления правил фильтрации

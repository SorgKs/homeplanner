...
4. **КРИТИЧЕСКИ ВАЖНО: Запускать тесты ТОЛЬКО ОДИН РАЗ и дожидаться полного завершения**. 
   - **НЕПРАВИЛЬНО**: запускать тесты несколько раз подряд, прерывать выполнение, перезапускать при отсутствии немедленного вывода.
   - **ПРАВИЛЬНО**: запустить тесты один раз и дождаться полного завершения выполнения. Тесты могут выполняться несколько секунд или минут — это нормально. Вывод результатов появится после завершения всех тестов.
   - Если команда запущена и выполняется — **НЕ ПРЕРЫВАТЬ** и **НЕ ЗАПУСКАТЬ ПОВТОРНО**. Дождаться завершения и получить полный отчет.
...
- **Проверка результатов**: при анализе логов первым шагом убедитесь, что тестовый прогон завершился полностью — в выводе должны присутствовать финальные строки с общей статистикой (`=== ... in XXs ===`, количество `PASSED/FAILED/SKIPPED`). Частичные логи или обрезанные выводы недопустимы для принятия решений о качестве.
...
- Последовательность локального запуска python-тестов:
  ...
  2. Запустить базовый прогон одним из двух способов:
     - `uv run pytest` (или `uv run pytest -n auto` для параллельного запуска)
     - или `pytest` (или `pytest -n auto` для параллельного запуска)
  3. Для покрытия перед публикацией — аналогично:
     - `uv run pytest --cov=backend --cov-report=term-missing`
     - или `pytest --cov=backend --cov-report=term-missing`
  4. Для максимальной производительности рекомендуется использовать параллельный запуск:
     - `pytest -n auto` (автоматическое определение количества воркеров)
     - или `pytest -n 4` (указать количество воркеров вручную)
...
Для ускорения выполнения тестов используется параллельный запуск через `pytest-xdist`:

```bash
# Установка
pip install pytest-xdist

# Запуск с автоматическим определением количества воркеров
pytest tests/ -v -n auto

# Запуск с указанным количеством воркеров
pytest tests/ -v -n 4

# Запуск с профилированием в параллельном режиме
pytest tests/ -v -n auto --profile
```

**Результаты оптимизации:**
- Время выполнения: с 13.18 до 7.74 секунд (ускорение в 1.7 раза)
- Количество воркеров: автоматически определяется по количеству CPU ядер
- Изоляция: каждый воркер использует свою in-memory БД, тесты полностью изолированы

**Важно:** При использовании параллельного запуска убедитесь, что:
- Тесты не зависят от глобального состояния
- Фикстуры с `scope="module"` корректно работают в параллельном режиме
- In-memory SQLite используется для изоляции БД между воркерами

### Разработка новых тестов

1. **Определите цель теста.** Чётко зафиксируйте бизнес-требование или сценарий, который покрывает тест. При необходимости добавьте описание в `docs/TASK_HISTORY.md`.
2. **Выберите структуру.** Для backend-тестов используйте паттерны `db_setup` + `db_session` из существующих файлов, чтобы тесты работали на in-memory SQLite.
3. **Соблюдайте типизацию и docstring.**
   - Каждая функция и фикстура должны иметь аннотации типов и docstring в формате PEP 257.
   - В тестах с дополнительными типами импортируйте их внутри `if TYPE_CHECKING:`.
4. **Используйте общие утилиты.**
   - `tests/utils.py` и `tests/utils/` содержат фабрики и вспомогательные функции.
   - При необходимости расширяйте их, чтобы избегать дублирования кода.
5. **Минимизируйте состояние.**
   - Для HTTP-клиентов задействуйте `test_client_with_session`.
   - Для моков используйте `pytest-mock` (`MockerFixture`).
6. **Проверяйте производительность.**
   - Для новых тестовых модулей прогоните `pytest <path> -v -n auto`.
   - При появлении узких мест используйте `pytest --profile --durations=10`.

### Поддержка существующих тестов

- **Перед правками** изучите связанные документы (`docs/TASK_HISTORY.md`, `docs/CHANGELOG_HISTORY.md`) и убедитесь, что тест отражает актуальную логику.
- **При обновлении фикстур** всегда проверяйте совместимость с параллельным запуском (`-n auto`), чтобы избежать гонок данных.
- **Дублирование сценариев** исключается: если тест частично пересекается с другим, расширьте существующий вместо создания нового.
- **Рефакторинг** выполняйте небольшими шагами:
  1. Сначала обновите тесты (структура, фикстуры).
  2. Затем запустите `pytest <модуль> -v -n auto`.
  3. После успешного прогона выполните полный `pytest tests/ -v -n auto`.
- **Документируйте изменения** в `docs/CHANGELOG_HISTORY.md`, если тест закрывает конкретную задачу или фиксирует найденную регрессию.

### Регламент запусков

| Задача                              | Команда                                               | Примечание                             |
|------------------------------------|--------------------------------------------------------|----------------------------------------|
| Быстрый smoke-прогон               | `pytest tests/ -v -n auto`                             | Использует все доступные ядра          |
| Прогон одного модуля               | `pytest tests/test_events.py -v -n auto`               | Аналогично для любых файлов            |
| Покрытие перед релизом             | `pytest tests/ -v -n auto --cov=backend --cov-report=term-missing` | Совместимо с параллельным запуском |
| Профилирование                     | `pytest tests/ -v -n auto --profile --durations=10`    | Выявляет узкие места                   |
| Диагностика конкретного теста      | `pytest tests/test_tasks_router.py -k test_get_today_task_ids -vv --maxfail=1` | Удобно для детального анализа   |

**Требование:** перед пушем обязательно выполнить хотя бы `pytest tests/ -v -n auto`. Если тесты падают, сначала зафиксируйте причину (лог/issue), затем согласуйте план исправлений.

#!/usr/bin/env python3
"""Fix BinaryLogger.log calls by correcting params and adding types."""
...
def fix_file(file_path: Path):
    """Fix BinaryLogger.log calls in a file."""
    filename = file_path.name
    if filename not in FILE_CODES:
        print(f"Warning: {filename} not in FILE_CODES, skipping")
        return
    ...
    # Fix emptyList(, fileCode, fileCode, fileCode) to emptyList<Any>(), fileCode
    pattern_empty = rf'emptyList\(\s*,\s*{file_code}\s*,\s*{file_code}\s*,\s*{file_code}\s*\)'
    replacement_empty = f'emptyList<Any>()'
    
    # Fix listOf(..., fileCode, fileCode, fileCode) to listOf<Any>(...)
    # More complex: find listOf( something , fileCode, fileCode, fileCode)
    # Use regex to capture the content before the trailing fileCodes
    pattern_listof = rf'listOf\(\s*([^)]*?)\s*,\s*{file_code}\s*,\s*{file_code}\s*,\s*{file_code}\s*\)'
    def repl_listof(match):
        inner = match.group(1).rstrip(',').strip()
        return f'listOf<Any>({inner})'
    
    new_content = re.sub(pattern_empty, replacement_empty, content)
    new_content = re.sub(pattern_listof, repl_listof, new_content)

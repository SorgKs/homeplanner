...
#### Рекомендуемый способ: Использование build.sh (Linux/Mac)

Для автоматического увеличения номера патча и отслеживания изменений приложения используйте скрипт `build.sh`:

```bash
# Сборка debug версии (рекомендуется для разработки)
./build.sh --debug
...
**Важно**: Скрипт `build.sh` автоматически:
- Увеличивает номер патча в `version.json` на 1 перед каждой сборкой
- Это позволяет отслеживать изменения приложения по номерам версий
- Собранный APK будет иметь версию в формате `MAJOR.MINOR.PATCH` (например, `0.2.198`)

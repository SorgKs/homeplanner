...
interface TaskCacheDao {
    ...
    // Актуальные (active = 1) должны храниться в оффлайн-режиме "вечно".
    @Query("SELECT * FROM tasks_cache WHERE updatedAt < :cutoffTime AND active = 0 ORDER BY lastAccessed ASC LIMIT :limit")
    ...
    // Удаляем по времени только неактуальные задачи, чтобы active = 1 не терялись из оффлайн-кэша.
    @Query("DELETE FROM tasks_cache WHERE updatedAt < :cutoffTime AND active = 0")

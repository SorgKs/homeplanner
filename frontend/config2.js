...
(function configureHomePlanner() {
    ...
    function loadText(path) {
        console.log("loadText called with path:", path);
        const xhr = new XMLHttpRequest();
        xhr.open("GET", path, false);
        xhr.overrideMimeType("text/plain");
        xhr.send(null);
        if (xhr.status >= 200 && xhr.status < 300 && xhr.responseText) {
            console.log("loadText loaded content length:", xhr.responseText.length);
    ...
    function parseToml(content) {
        console.log("parseToml called with content length:", content.length);
        const result = {};
    ...
    let settings = {};
    try {
        console.log("Loading settings.toml");
        const tomlContent = loadText("/common/config/settings.toml");
        console.log("Loaded tomlContent sample:", tomlContent.substring(0, 100));
